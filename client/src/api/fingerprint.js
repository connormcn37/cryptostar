...
import { LOG, CLIENT_HOST } from "./logremote";
...
const fp_mime = () => {
  try {
    ...
    const strOut = md5(a);
    if (DEBUG)
      LOG.log("[" + CLIENT_HOST + "]", "1.fingerprint fp_mime : ", strOut);
    //    if ( strOut === "1ed7a85b37844b0b9d8b9e09d3d1305c" )
    //      LOG.log(
    //        "[" + CLIENT_HOST + "]",
    //        "2.fingerprint fp_mime VERIFY : ", strOut === "1ed7a85b37844b0b9d8b9e09d3d1305c"
    //      );
    return strOut;
  } catch (e) {}
};
const fp_plugins = () => {
  try {
    ...
    const strOut = md5(a);
    if (DEBUG)
      LOG.log("[" + CLIENT_HOST + "]", "1.fingerprint fp_plugins : ", strOut);
    //    if ( strOut === "388cc04da50a2ed7637ae7739abb9039" )
    //      LOG.log(
    //        "[" + CLIENT_HOST + "]",
    //        "2.fingerprint fp_plugins VERIFY : ", strOut === "388cc04da50a2ed7637ae7739abb9039"
    //      );
    return strOut;
  } catch (e) {}
};
...
export const Fingerprint = cb => {
  ...
  const fingerprint_plugins = fp_plugins();
  const fingerprint_mime = fp_mime();
  fp_fp2((err, result) => {
    let fp2;
    if (err) {
      if (DEBUG)
        LOG.log("[" + CLIENT_HOST + "]", "1.tasks fingerprint error : ", err);
      fp2 = err;
    } else {
      fp2 = result;
      payload = {
        ...
        mime: fingerprint_mime
      };
      const payloadMd5 = md5(payload.fp2 + payload.navigator + payload.fonts);
      payload.signature = payloadMd5;
      if (DEBUG)
        LOG.log(
          "[" + CLIENT_HOST + "]",
